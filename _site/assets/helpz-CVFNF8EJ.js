...
;
import { m as mirrorId, H as HighlightJS } from "./help-BFUtjB_U.js";
import "./_virtual_jekyll-config-DIUmS6FF.js";
...
function generateFormConfig(form) {
  ...
  for (const x in formData) {
    ...
    if (!varConf)
      continue;
    let optConf = null;
    if ("option" in varConf)
      optConf = varConf.option[formData[x]];
    else if ("true" in varConf || "false" in varConf) {
      optConf = formData[x] === "on" ? varConf.true : varConf.false;
    }
    if (typeof optConf === "object")
      Object.assign(conf, optConf);
...
function renderCode(tmpl) {
  ...
  if (form)
    Object.assign(conf, generateFormConfig(form));
  conf.endpoint = conf.scheme + "://" + conf.host + conf.path;
  let rendered = mustache.render(tmpl.textContent.trim(), conf, {}, { escape: (x) => x });
  try {
    const lang = tmpl.attributes.getNamedItem("z-lang");
    if (lang && HighlightJS.getLanguage(lang.value)) {
      rendered = HighlightJS.highlight(rendered, { language: lang.value }).value;
    }
  } catch (err) {
    console.error(err);
  }
...
//# sourceMappingURL=helpz-CVFNF8EJ.js.map
