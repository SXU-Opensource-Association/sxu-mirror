...
;
import { m as mirrorz_help_link, h as hostname } from "./_virtual_jekyll-config-DIUmS6FF.js";
import { T as TUNASYNC_JSON_PATH, o as options } from "./_virtual_jekyll-data-NZIznc-5.js";
import { h as getDefaultExportFromCjs } from "./default-NUifn3ep.js";
...
const deprecated = (version2, message) => {
  if (seenDeprecations["".concat(version2, "/").concat(message)]) return;
  console.log("Deprecated as of ".concat(version2, ". ").concat(message));
  seenDeprecations["".concat(version2, "/").concat(message)] = true;
};
  ...
  function _highlight(languageName, codeToHighlight, ignoreIllegals, continuation) {
    ...
    function processLexeme(textBeforeMatch, match) {
      ...
      if (lastMatch.type === "begin" && match.type === "end" && lastMatch.index === match.index && lexeme === "") {
        ...
        if (!SAFE_MODE) {
          const err = new Error("0 width match regex (".concat(languageName, ")"));
...
function python(hljs) {
  ...
  const lookahead2 = "\\b|".concat(RESERVED_WORDS.join("|"));
  const NUMBER = {
    className: "number",
    relevance: 0,
    variants: [
      ...
      {
        begin: "(\\b(".concat(digitpart, ")|(").concat(pointfloat, "))[eE][+-]?(").concat(digitpart, ")[jJ]?(?=").concat(lookahead2, ")")
      },
      {
      ...
      // https://docs.python.org/3.9/reference/lexical_analysis.html#imaginary-literals
      {
        begin: "\\b([1-9](_?[0-9])*|0+(_?0)*)[lLjJ]?(?=".concat(lookahead2, ")")
      },
      {
        begin: "\\b0[bB](_?[01])+[lL]?(?=".concat(lookahead2, ")")
      },
      {
        begin: "\\b0[oO](_?[0-7])+[lL]?(?=".concat(lookahead2, ")")
      },
      {
        begin: "\\b0[xX](_?[0-9a-fA-F])+[lL]?(?=".concat(lookahead2, ")")
      },
      // imagnumber (digitpart-based)
      // https://docs.python.org/3.9/reference/lexical_analysis.html#imaginary-literals
      {
        begin: "\\b(".concat(digitpart, ")[jJ](?=").concat(lookahead2, ")")
      }
    ]
  };
  const COMMENT_TYPE = {
...
HighlightJS.registerAliases("text", { languageName: "plaintext" });
...
(function(module) {
  var Mark2 = {
    ...
    _pipe: function(val, expressions) {
      ...
      if (expression = expressions.shift()) {
        ...
        fn = parts.shift().trim();
        try {
          result = Mark2.pipes[fn].apply(null, [val].concat(parts));
    ...
    _eval: function(context, filters, child) {
      var result = this._pipe(context, filters), ctx = result, i = -1, j, opts;
      if (result instanceof Array) {
        ...
        while (++i < j) {
          ...
          result += child ? Mark2.up(child, ctx[i], opts) : ctx[i];
        }
      } else if (result instanceof Object) {
        result = Mark2.up(child, ctx);
      }
      return result;
    },
    // Process the contents of an IF or IF/ELSE block.
    _test: function(bool, child, context, options2) {
      var str = Mark2.up(child, context, options2).split(/\{\{\s*else\s*\}\}/);
    ...
    _bridge: function(tpl, tkn) {
      ...
      var exp = "{{\\s*" + tkn + "([^/}]+\\w*)?}}|{{/" + tkn + "\\s*}}", re = new RegExp(exp, "g"), tags = tpl.match(re) || [], t, i, a = 0, b = 0, c = -1, d = 0;
  ...
  Mark2.up = function(template, context, options2) {
    context = context || {};
    options2 = options2 || {};
    var re = /\{\{(.+?)\}\}/g, tags = template.match(re) || [], tag, prop, token, filters = [], selfy, testy, child, result, global, include, ctx, i = 0, j = 0;
    if (options2.pipes) {
      this._copy(options2.pipes, this.pipes);
    }
    if (options2.includes) {
      this._copy(options2.includes, this.includes);
    }
    if (options2.globals) {
      this._copy(options2.globals, this.globals);
    }
    if (options2.delimiter) {
      this.delimiter = options2.delimiter;
    }
    if (options2.compact !== void 0) {
      this.compact = options2.compact;
    }
    while (tag = tags[i++]) {
      result = void 0;
      child = "";
      ...
      if (/^\{\{\s*else\s*\}\}$/.test(tag)) {
        continue;
      } else if ((global = this.globals[prop]) !== void 0) {
        result = this._eval(global, filters, child);
      } else if (include = this.includes[prop]) {
        if (include instanceof Function) {
          include = include();
        }
        result = this._pipe(Mark2.up(include, context, options2), filters);
      } else if (prop.indexOf("#") > -1) {
        options2.iter.sign = prop;
        result = this._pipe(options2.iter, filters);
      } else if (prop === ".") {
        result = this._pipe(context, filters);
      } else if (prop.indexOf(".") > -1) {
        prop = prop.split(".");
        ctx = Mark2.globals[prop[0]];
        if (ctx) {
          j = 1;
        } else {
          j = 0;
        ...
        result = ctx ? Mark2.up(child, ctx) : void 0;
      ...
      if (testy) {
        result = this._test(result, child, context, options2);
      }
      template = template.replace(tag, result === void 0 ? "???" : result);
    }
  ...
  Mark2.pipes = {
    ...
    more: function(a, b) {
      return Mark2._size(a) > b ? a : false;
    },
    less: function(a, b) {
      return Mark2._size(a) < b ? a : false;
    },
    ...
    notlike: function(str, pattern) {
      return !Mark2.pipes.like(str, pattern) ? str : false;
    },
    upcase: function(str) {
    ...
    sort: function(arr, prop) {
      ...
      return [].concat(arr).sort(prop ? fn : void 0);
    },
    fix: function(num, n) {
...
const Mark = /* @__PURE__ */ getDefaultExportFromCjs(markupExports);
const mirrorId = JSON.parse(
  ((_a = Array.from(document.getElementsByTagName("script")).filter(
    (script) => script.type === "text/x-tuna-help-mirrorid"
  )[0]) == null ? void 0 : _a.textContent) || '""'
);
...
fetch(TUNASYNC_JSON_PATH).then((resp) => resp.json()).then((statusData) => {
  ...
  options.unlisted_mirrors.forEach((elem) => {
...
//# sourceMappingURL=help-BFUtjB_U.js.map
