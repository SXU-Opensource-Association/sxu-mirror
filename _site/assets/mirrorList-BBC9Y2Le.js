import { o as options, T as TUNASYNC_JSON_PATH } from "./_virtual_jekyll-data-NZIznc-5.js";
  ...
  const res = customRef((track2, trigger2) => {
    ...
    return {
      ...
      set(value) {
        ...
        i.emit("update:".concat(name), options2.set ? options2.set(value) : value);
      }
    };
  });
...
function processingHandlers(globalOptions) {
  ...
  const postProcessStatusData2 = (status_data, additional) => {
  ...
  const genMainMirrorList = (status_data, helpPages) => {
    return status_data.filter((d) => !(d.status == "disabled")).map((d) => {
      var _a;
      return {
        ...d,
        url: forceHelp[d.name] ? helpPages[d.name] : (_a = d.url) != null ? _a : "/".concat(d.name, "/"),
  ...
  return {
    postProcessStatusData: postProcessStatusData2,
    unlistedMirrors: unlisted,
    genMainMirrorList
  };
}
const { postProcessStatusData } = processingHandlers(options);
const useMirrorList = (additional = []) => {
...
//# sourceMappingURL=mirrorList-BBC9Y2Le.js.map
